...
public class Enemy {
	...
	public Enemy() {
		...
		size = 40 + ((int)Math.sqrt(Shooter2Player.difficultyLevel * 5));
		speed = 3 + ((int)Math.sqrt(Shooter2Player.difficultyLevel) * 5);
		health = 50 + (Shooter2Player.difficultyLevel * 2);
		points = 100 + (Shooter2Player.difficultyLevel * 5);
		damage = 20 + (Shooter2Player.difficultyLevel * 2);
		sizeChange = 5;
		...
		if ((int)(Math.random()*2) == 0) {
			if ((int)(Math.random()*2) == 0) {
				x = 0;
				y = (int)(Math.random()*Shooter2Player.WINDOW_HEIGHT);
				dir = Shooter2Player.RIGHT;
			}
			else {
				x = (int)Shooter2Player.WINDOW_WIDTH;
				y = (int)(Math.random()*Shooter2Player.WINDOW_HEIGHT);
				dir = Shooter2Player.LEFT;
			}
		}
		else {
			if ((int)Math.random()*2 == 0) {
				x = (int)(Math.random()*Shooter2Player.WINDOW_WIDTH);
				y = 0;
				dir = Shooter2Player.DOWN;
			}
			else {
				x = (int)(Math.random()*Shooter2Player.WINDOW_WIDTH);
				y = (int) Shooter2Player.WINDOW_WIDTH;
				dir = Shooter2Player.UP;
			}
		}
	}
