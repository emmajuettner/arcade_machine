...
public class Player {	
	...
	public void move(int targetX, int targetY) {
		...
		*/
		...
		if (x>Shooter.WINDOW_WIDTH-size-1) {
			x=(int)Shooter.WINDOW_WIDTH-size-1;
		}
		if (y<0) {
			y=0;
		}
		if (y>Shooter.WINDOW_HEIGHT-size-1) {
			y=(int)Shooter.WINDOW_HEIGHT-size-1;
		}
	}
	...
	public void drawHealth(Graphics g) {
		...
		g.setFont(new Font("TimesRoman", Font.PLAIN, (int)Shooter.WINDOW_HEIGHT/50));
		...
		if(playerNumber == 1) { //right player
			g.drawString("Health: ", (int)Shooter.WINDOW_WIDTH-10-health_width-100-10-lives_width-10-3*(height+5), 10+height);
			g.drawRect((int)Shooter.WINDOW_WIDTH-100-10-lives_width-10-3*(height+5), 10, 100, height); //empty health box
			g.fillRect((int)Shooter.WINDOW_WIDTH-100-10-lives_width-10-3*(height+5), 10, health, height); //fill in current health
		}
	...
	public void drawLives(Graphics g) {
		...
		if(playerNumber == 1) { //right player
			g.drawString("Lives: ", (int)Shooter.WINDOW_WIDTH-lives_width-10-3*(height+5), 10+height);
			for (int i = 0; i < lives; i++) {
				g.fillOval((int)Shooter.WINDOW_WIDTH-10-(3-i)*(height+5), 10, height, height);
			}
		}
		if(playerNumber == 2) { //left player
	...
	public void drawScore(Graphics g) {
		...
		if(playerNumber == 1) { //right player
			g.drawString("Score : " + score, (int)Shooter.WINDOW_WIDTH-10-score_width-10, 10+height+height);
		}
		if(playerNumber == 2) { //left player
